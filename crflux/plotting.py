...
def generate_model_comparison_plots(save_dir=None):
    ...
    try:
        ...
        pmodels.append(
            (
                lambda opts: mods.GlobalSplineFit2025(
                    version="2025",
    ...
    try:
        _ = mods.GlobalSplineFit2025(version="2019", time_interval=time_interval)
        pmodels.append(
            (
                lambda opts: mods.GlobalSplineFit2025(
                    version="2019",
    ...
    try:
        _ = mods.GlobalSplineFit2025(version="2017", time_interval=time_interval)
        pmodels.append(
            (
                lambda opts: mods.GlobalSplineFit2025(
                    version="2017",
    ...
    # Try to add PCAGlobalSplineFit if available
    try:
        _ = mods.PCAGlobalSplineFit()
        pmodels.append(
            (lambda opts: mods.PCAGlobalSplineFit(), None, "PCA-GSF", "orange", "-")
        )
    except (ImportError, AttributeError, ModuleNotFoundError, FileNotFoundError) as e:
        warnings.warn(f"PCAGlobalSplineFit not available: {e}", ImportWarning)
    ...
    for mclass, moptions, mtitle, color, ls in pmodels:
        ...
        # Skip lnA for GlobalSplineFitBeta and PCAGlobalSplineFit as they don't support it
        # but GlobalSplineFit2025 does support lnA
        if isinstance(pmod, (mods.GlobalSplineFitBeta, mods.PCAGlobalSplineFit)):
            continue
        try:
    ...
    # Filter out GlobalSplineFitBeta and PCAGlobalSplineFit for total flux and lnA plots
    # (these models don't support nucleus-level flux, only nucleon flux)
    # (GlobalSplineFit2025 supports these methods)
    pmodels_filtered = [
        m for m in pmodels if "GSF spl" not in m[2] and "PCA-GSF" not in m[2]
    ]
    ...
    if gsf2025_available and "GSF2025" in nucleon_flux:
        try:
            ...
            for mclass, moptions, mtitle, color, ls in pmodels:
                ...
                plt.plot(
                    evec_ratio, flux_ratio, color=color, ls=ls, lw=1.5, label=mtitle
                )
            ...
            if "GSF2025" in total_flux:
                ...
                plt.axhline(
                    y=1.0, color="k", ls="-", lw=1.5, label="GSF2025", alpha=0.5
                )
                plt.semilogx()
                plt.xlabel(r"$E_{particle}$ [GeV]")
                plt.ylabel("Flux ratio relative to GSF2025")
